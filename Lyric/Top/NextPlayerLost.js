    var REV ="Beta(IN WORK)"//this revision
...
function applyPx(){
    //alert(ARRbars)
    //alert("applyPx() "+SCROLLpix+" > "+ BARS);
    //var tot=0
    var newPt
    for (i=0;i<ARRbars.length;i++){
        newPt=parseFloat(SCROLLpix/BARS,10)*ARRbars[i]
        //if (i===15) {alert(newPt)}
        ARRbars[i]=newPt;}
        //if (i===15) {alert(ARRbars[i])}
     //alert(ARRbars.join('\n'))  ;
     statusMsg("Applied pixels to ARRbars...(ex bar 15:"+ARRbars[15]+")",0)}   

function showBAR() {
    BAR=0;
    for(i=0;ARRbars[i]<=Ypos;i++){
        BAR=BAR+1}
        BARSTOP=ARRbars[BAR]}
        /*
        barId=('mit'+parseInt(BAR+1));
        lastBarId=('mit'+parseInt(BAR));
        //document.getElementById('mit'+parseInt(i)).style.backgroundColor='transparent'
        statusMsg("NEXT BAR: "+barId+", marked at "+parseInt(ARRbars[BAR],10)+" pixels from "+parseInt(Ypos,10)+ " pixels",0)
       // statusMsg("BARSTOP: "+ BARSTOP,0);
        document.getElementById(barId).style.backgroundColor='lightgreen';
        if(BAR>0){document.getElementById(lastBarId).style.backgroundColor='transparent';}
        }
...
function presets(){
    ...
    var arrPresets="CAPS,HILITE,FULLscreen,LEFTborder,CLOCK,TEXT,SHADE,NOTES,SETnotes,POPnotes,TECHnotes,AUTOnext,COUNTin,CLICKER,BREAKlines,BIGchords,LOOPER,LINEtime,MITCH".split(',');
    if (PRESET==="Default Mode"){arrTF="true,false,false,false,false,false,false,false,false,false,false,false,false,false,true,false,false,false,false".split(',');
        setLineLimit(90)}
    else if (PRESET==="Perform Mode"){arrTF="true,false,true,false,true,false,true,false,true,false,false,true,true,false,true,true,false,false,false".split(',');
        setLineLimit(50)}
    else if (PRESET==="Practice Mode"){arrTF="true,false,true,false,true,false,false,false,true,false,false,true,true,true,true,true,true,true,true".split(',');
        setLineLimit(55)}
    else if (PRESET ==="Developer Mode"){arrTF="true,false,false,false,true,false,false,false,false,false,false,false,false,false,false,false,true,true,true".split(',');
        setLineLimit(90);statusMonitor(1);
        document.getElementById('soundSelector').selectedIndex=1;setSoundModeDefault(document.getElementById('soundSelector').value);
        document.getElementById('mySet').selectedIndex=5;selectTune('Pancho and Lefty')}
...
function durCalc(){
    ...
    DURsource="Undefined";
    DURcalc=parseFloat(BARS*BEATS*60/BPM,10);//^ alert(DURcalc);
...
function arrConvert(){//alert('arrConvert()'); //^ Setup to walk thru the ARRLines
    ...
    while(j < ARRlines.length){//^ Walk through the ARRlines to build the htmlStrings  
        ...
        if(lType =='irealb'){
            ...
            if(LINEnum ===true||LINEtime===true){NewLine="<X51>    </X51>" + NewLine;}  
...
if (MITCH===true){
    ...
    while(n<NewLine.length){
        if (NewLine[n]==="<"){htm=true}
        else if (NewLine[n]===">"){htm=false}
        else if (htm===true) {htm=true}
        else if(htm!=true){
            if(NewLine[n]=="|"){
                count=count +1
                if (firstChord===true){
                    barLine=barLine+"<a id=\'mit"+count +"\'>|"
                    firstChord=false}
                else{
                    barLine=barLine+"</a><a id=\'mit"+count+"\'>|"}}
            else{barLine=barLine+NewLine[n]}}
        n =n+1;}
    while (barLine.length < LONGLINE){barLine=barLine +" ";}
    barLine=barLine+"               </a>"
    NewLine="<X2>"+barLine+"</X2>"//add color back in
...
//==================================================================
            ...
            if(LINEnum ===true){    
                if(lineNum <10) {num ="<X51>"+lineNum + "   </X51>";}
                else if(lineNum <100) {num ="<X51>"+lineNum + "  </X51>";}
                else {num =+"<X51>"+lineNum + " </X51>";}
                NewLine=num + NewLine;
                lineNum=lineNum+1;}
            if(LINEtime ===true){  
                NewLine="<X51>" + secToMin(((lineNum-1)/lyricLines)*DUR,10)+ "</X51>"+NewLine;
                lineNum=lineNum+1;}}
...
//*PLAYER ROUTINES====================================================AAA
function clearMitch(){
    for(i=1;i<BARS-2;i++){document.getElementById('mit'+i).style.backgroundColor='transparent'}//based on scroll time
    statusMsg("MITCH > Cleared the BARS...",0)}

/*function  mitchReset(){//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ mitchReset
    if (MITCH===true) {
        clearMitch();}
    var pct=parseFloat(document.getElementById("Tune").scrollTop/SCROLLpix,0);///get pos-tion
    MITCHcount=parseInt(parseInt(BARS,0)*parseFloat(pct,10),10);
    MITCHint=parseFloat(SCROLLpix/BARS,10);
    statusMsg("RESET: Position: BAR:" +MITCHcount+ " of "+BARS+ " at "+MITCHint+ " milliseconds Per BAR",0)}*/
...
function scrollEngine(){//^ the actual scrolling routine keep it simple* before it starts SCOLLstartTime,SCROLLkon must be set
    ...
    if(Ypos>SCROLLpix-2){SCROLLstate='pause'}
    if (SCROLLstate==='pause') {//alert('Pause')
        clearTimeout(TIMEOUTscroll);
        trackPause();return}
    else{//move & check for end
        Ypos=document.getElementById("Tune").scrollTop;//get Position
        if(Ypos==TOPlast) {SS=SS+1}else{SS=0;TOPlast=Ypos}//looend of scroll)
        ...
        //else{//^ STD SCROLL move part 2
            ...
            if (isNaN(BARSTOP)) {BARSTOP=0}
            if (BARSTOP<Ypos){ showBAR()};
        songTime=parseFloat(new Date().getTime()- SCROLLstartTime,10);
        
        
        newPos=parseInt((songTime/SCROLLkon)+Ystart,10);

        document.getElementById("Tune").scrollTop= newPos //move to new position
        if(document.getElementById("Tune").scrollTop !== newPos&SS>5){Yend =true;}//wait 6 non moves to declare stop
        if(LOOPER===true & document.getElementById("Tune").scrollTop >=LOOPend){Yend =true;}//find loop end
        TIMEOUTscroll=setTimeout(function(){scrollEngine();},SCROLLkon);}}}

function scrollEngine2(){//^ the actual scrolling routine keep it simple* before it starts SCOLLstartTime,SCROLLkon must be set
    statusMsg('scrollEngine('+Ypos+'>>>'+SS+')',0)
    if (SCROLLstate==='pause') {
        clearTimeout(TIMEOUTscroll);
        trackPause()}
...
function setCustom(VAR,val,action){
    ...
    if(VAR=='BREAKlines') {//works with BIGchords 
        ...
        if(BREAKlines===true){//turn on BIGchords too
            statusMsg("--Custom E: "+VAR+": forcing BIG chords to true "+action,0)
            BIGchords=true
        ...
        while (n < line.length){//^ step through the line
            if(line[n]==" "){
                if(chord.length ===0){//^ not working a chord so work space
                    ...
                    chordNew ="";}}
            else{chord = chord + line[n];
                if(n==line.length-1){lineNew = lineNew + chordTranspose(chord, steps);}}//^ end of the line but no space to change
            n=n+1;}}
    return lineNew;}
    ...
    if(steps===0) {
        ...
        for (n=0; n < chord.length;){
            ...
            if(chord[n]=="A"|chord[n]=="B"|chord[n]=="C"|chord[n]=="D"|chord[n]=="E"|chord[n]=="F"|chord[n]=="G"){
                if(n < chord.length-1 &&(chord[n+1]=="#"|chord[n+1]=="b")){//^ ifnot at the end of the chord check for next part of note
...
function analizeSong(){
    ...
    str = str +"<br>======================<br>SCROLL INFO<pre>";
    str = str +"<br>  SCROLL PIX:      "+ SCROLLpix+ "px; (global variable:Window Height)"
    str = str +"<br>  BAR (current)    "+ BAR+ "(global variable: current BAR Number set by scrollEngine )"   
    str = str +"<br>  BARS (current)   "+ BARS+ "(global variable:SUM of whole and half bars)"    
    str = str +"<br>  SCROLL RATE      "+ parseInt(SCROLLkon,10) + " micro-sec/pixel  (DUR/SCROLLpix)";
    str = str +"<br>  SCROLL POSITION  "+ document.getElementById("Tune").scrollTop +"px; (Current Y position)</pre>"
...
function countBARS(){
    ...
    var typ;
    var sum=0
    var j =0;var k=0;BARS=0;BARSperLine=0;//ARRbars="".split(',')
    ...
    statusMsg("BARS: "+BARS+"    CHORD LINES: "+CHORDlines+"   BARS/LINE: "+BARSperLine+"   (half BARS: "+BARShalf+")",0)}//get the std number of bars from the first line}
