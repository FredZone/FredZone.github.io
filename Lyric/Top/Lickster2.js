...
function scrollHor(dir)
    {
        Xstart=document.getElementById("screen").scrollLeft;
        Xend=false;
        SCROLLstartTime=new Date().getTime();
        scrollHorEngine(dir);
    }

function scrollHorEngine(dir)//* the actual scrolling routine keep it simple
    {   //* before it starts SCOLLstartTime,SCROLLkon must be set
        //alert(1)
        var nowTime = new Date().getTime(); 
        Xpos = document.getElementById("screen").scrollLeft; statusMsg("SL: "+Xpos,'green');
        if(Xend===true) statusMsg('over','red');
       
        else//* STD SCROLL move part 2
        {
            var newPos = parseInt(((nowTime - SCROLLstartTime)/(dir*SCROLLkon))+Xstart,10);
            document.getElementById("screen").scrollLeft= newPos;//* MMM
            if (document.getElementById("screen").scrollLeft !== newPos){Xend =true;}//actual end of tune
            TIMEOUTscroll = setTimeout(function(){scrollHorEngine(dir);},SCROLLkon);
        }
    }
...
window.onload = function()
    {   statusMsg('Loading javascript...','black');
        ...
        LICKfileName='12BarBluesA';
        document.getElementById('fileInput').addEventListener('change', fileRead, false);//fileReadSetup
        statusMsg('Loading Default Lick Set...'+LICKfileName);
        loadServerLickFile();
        statusMsg('READY');
    };

function loadServerLickFile()
    {  statusMsg('Loading the Lick File: '+ LICKfileName);
        changesProtect(false);
        BACK=title;
        CTO();
        var path ="../Licks/" + LICKfileName + "/" + LICKfileName + ".txt";  //* get the text file
        var request = new XMLHttpRequest();
        request.open("GET", path, false);
        request.send(null);
...
function loadServerInfoFile() //entry with jTitle
    {
        var tempo=120;
        ...
        var path ="../Licks/"+LICKfileName+"/" + LICKfileName + ".inf";  //* get the text file
        var request = new XMLHttpRequest();
        request.open("GET", path, false);
        request.send(null);
...
function loadServerBackTrack() //entry with jTitle
    {
        statusMsg('Loading the Backing Track...');
        document.getElementById('Audio2').src = "../Licks/"+LICKfileName+"/" + LICKfileName + ".mp3";//}//as soon as possible}
        statusMsg('Ready...');
    }
...
function lickSelect(indx)
    {
        ...
        if (MODE==='strum') {mode('tab');}
        if (EDITED===true)
            {vis('warning','visible');
            //TIMEOUTscroll=setTimeout(function(){trackPlay();},2000);
            setTimeout(function(){vis('warning','hidden');},2000);}
...
function lickRender()
    {
        ...
        while(b < ARRsteps.length)  //step  
        {
            ...
            txtNew="<a onclick='selectPart("+b+")'>"+ txtNew +"</a>";
            if (parseInt(b/BEATS,10)==parseFloat(b/BEATS,10)) {txtNew=txtNew+"|";}
            
            if (b>=PARTA & b<=PARTB) {txtNew="<X5>"+txtNew+"</X5>";}
            txt = txt+txtNew;
            b = b+1;
        }
        txt = txt+"</div>";//alert(txt);
...
function fileRead(e)
    ...
    //<input onclick = "fileReadSetup()" type="file" id = "fileInput" style = "position:fixed; visibility:hidden; left:0%; height:0%; width:0%; top:0%"> 
    {
        ...
        reader.onload = function(e)
        {
            ...
            LICKfileName = (file.name.substring(0,(file.name.length)-4));
            ...
            loadJam();
            alert(1);
            statusMsg ("FILE '"+ LICKfileName +"' LOADED");
        };
...
function fileLickSave()
    ...
    fileSaveAsText(LICKfileName+".txt",str);
    }
...
function lickNew()
    {//ARRsteps[0]=TITLE+";"+ROOT+";"+BEATS+";"+(INFO.split("/n")).join(" ")+";"+BACK+";"+LINK+";"+AND;
        ARRlicks.splice(ARRlicks.length,0,"NEW-LICK;C;4;New;;;A;@@@@");
        lickSelector();
        ARRsteps="";
        lickSelect(ARRlicks.length-1);
...
function mode(m)

    {
        ...
        document.getElementById('buttonStrum').style.opacity=0.3;
        if (m =='tab')
        {   
        ...
        {
           //jTrackSet(LICKfileName);
            document.getElementById('buttonStrum').style.opacity=1;
            dis('strumHolder','block');
            statusMsg ("Strum Mode");
            renderStrum();
        } 
    }
...
function stepRender(STEP)//set WorkGlobals render the string
    ...
    //see if there is a pick notation============================================
                ...
                if (wire==7)
                {
                    statusMsg(arrStrings[j].split(';')[1]);
                    document.getElementById('w7').innerHTML=arrStrings[j].split(';')[1];
...
function renderStrum()
    {   var oneStep= "";
        var pct =5;
        statusMsg("Rendering Strum screen...");
        var step=1;
        var thing="";
        var j;
        var wireTop;
        ...
        if (AND==='C'){stp="X,1,&,&,2,&,&,3,&,&,4,&,&,5,&,&,6,&,&,7,&,&,8,&,&".split(",");}
        if (AND==='D'){stp="X,1,&,&,&,2,&,&,&,3,&,&,&,4,&,&,&,5,&,&,&,6,&,&,&,7,&,8,&,&,&".split(",");}
        if (ARRsteps.length >20)
        {
            pct=parseInt(100/(ARRsteps.length),10);
            document.getElementById("strumScreen").style.width='200%';
        }
        while(step<ARRsteps.length)
        {
            cnt = stp[c];
            if (parseInt(step/BEATS,10)==parseFloat(c/BEATS,10)) {c=0;}
            if (c==1) {cnt="<X4>"+cnt+"</X4>";}
            c = c+1;

            if (ARRsteps[step].length===0)//HANDLE NULL STRING==========================================
            {
                //oneStep="<div id='Step"+step +"' style='font-size:4vh;border-style:solid; background-color:transparent;height:100%; width:5%; position:absolute;top:0%;left:"+step*5+"%;z-index:200'>"+cnt+"</div>";
                oneStep="<div id='Step"+step +"' style='font-size:4vh;border-style:solid; background-color:transparent;height:100%; width:"+pct+"%; position:absolute;top:0%;left:"+step*pct+"%;z-index:200'>"+cnt+"</div>";
            }
            else//split step into substeps and store the string========================
            {
                arr=ARRsteps[step].split('|');
                oneStep="";
                //statusMsg('Strum at step '+step);
                j=0;
                arrStrings=ARRsteps[step].split('|');
                btm="";
                while(j<arrStrings.length)
                {
                    arr=arrStrings[j].split(';');//split into individual string descriptions 
                    wire=arr[0];
                    var pick = arrStrings[j][arrStrings[j].length-1];
                    if (wire>0)
                    {
                        if (pick=="U"|pick=="D"|pick=="T"|pick=="M"|pick=="I"|pick=="R"|pick=="L")
                        {
                            pick=pick+".png";
                        }
                        else
                        {
                            pick="transSquare.png";
                        }
                        if (wire ==1) {wireTop =22;}//0
                        else if (wire ==2) {wireTop =34;}//16
                        else if (wire ==3) {wireTop =46;}//33
                        else if (wire ==4) {wireTop =58;}//50
                        else if (wire ==5) {wireTop =70;}//67
                        else if (wire ==6) {wireTop =82;}//85
                        else if (wire ==7) {btm=arr[1];}//85
                        if (wire<7)
                        {
                            oneStep= oneStep + "\n<img src='../../Icons/Picking/"+pick+"' style='position:absolute; left:0%; top:"+wireTop+"%; left:20%; width:60%;'>";
                        }
                        j=j+1;
                    }    
                }
            //oneStep="<div id='Step"+step+"'style='border-style:solid; font-size:2vw; background-color:transparent;height:100%; width:5%; position:absolute;top:0%;left:"+step*5+"%;z-index:200'>"+step+"x"+'<br>'+btm+oneStep+"</div>";
            //oneStep="<div id='Step"+step+"'style='border-style:solid; font-size:2vw; background-color:transparent;height:100%; width:5%; position:absolute;top:0%;left:"+step*5+"%;z-index:200'>"+cnt+'<br>'+btm+oneStep+"</div>";
            oneStep="<div id='Step"+step+"'style='border-style:solid; font-size:2vw; background-color:transparent;height:100%; width:"+pct+"%; position:absolute;top:0%;left:"+parseFloat(step*pct,10)+"%;z-index:200'>"+cnt+'<br>'+btm+oneStep+"</div>";

            }
        thing=thing+oneStep;
        step=step+1;
        }
    document.getElementById("strumScreen").innerHTML=thing +"<img id='6strings' src='../../icons/6Strings.png' style='position:absolute; left:0%; top:25%;height:65%; width:100%;z-index:0'>";
    statusMsg(INFO);
    }
