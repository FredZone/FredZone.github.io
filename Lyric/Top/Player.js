...
function selectTune(titl)//* Entry Point using the TITLE (or extended title) to download tune
    {
        document.getElementById('myTune').innerHTML=((titl.split("|")[0]));//?wjhat is my tune??
        statusMsg("Downloading " + titl,'yellow');
        passCommand("Think");
        ALT =false;
        TITLEplus = titl;
        //TITLEplus is formated play info=@TITLE/KEY/BPM/SOUNDmode/NOTEset (@ indicates alternate if its present not used)
...
function arrConvert()//* J
    {   //* Setup to walk thru the ARRLines
        ...
        var bigFont=n.substring(0,4)+"vw";
        while(j < ARRlines.length)//* Walk through the ARRlines to build the htmlStrings  
        {
            ...
            {
                if (NOTEtech===undefined) {NOTEtech=NewLine.substr(3);}
                else
                {NOTEtech =NOTEtech +"\n"+NewLine.substr(3) ;}
            ...
            else if (lType =='chord')
            {
                ...
                var bars = arr.length;
                var barLen=parseInt(LONGLINE/(bars*BIGchordSize),10)-1;
                NewLine="";
                n=0;
                while(n<bars) {
                    temp = arr[n];
                   ...
                   // if (HILITE===false) {NewLine ="<X2A>" + NewLine + "</X2A>";}
                   if (HILITE===false) {NewLine ="<X99>" + NewLine + "</X99>";}
                    else {NewLine ="<X100>" + NewLine + "</X100>";}
                } //=============================================================    
                else
                {
                   NewLine ="<X2>" + NewLine + "</X2>";  
       ...
       // if (ALT ===true) {col = 'red'; tit = TITLE + " (ALTERNATE SONG)";}
        //htmlHead = htmlHead + col + ";font-size:8vh;line-height:100%;text-align:center}\nX99{color:green;font-size:10vh;line-height:150%;}\n</style></head><body>";
        ...
        //alert(htmlString);//DONT DELETE USE THIS TO DEBUG THE ARRconvert
        scrollSetup();
    }
...
function scrollSetup()//* get the song data,songHeight,iframeHeight,Duration,ScrollConstant Run after you set the content of page
    {   
        ...
        if(BOOT===true)
        {
            ...
            //if (document.fileModifiedDate) {statusMsg("Date Modified: "+ document.fileModifiedDate);}
        }
        else
        {
...
function endSong()
    {
        ...
        {wait= (SCROLLkon*WINDht*0.4);}//wait 60% screen before popping up top icons
...
//volSet(0);
            ...
            if(cmd == "DelayedPlay"){
                ...
                if (BIGbuttons===true) {dis('bigButtons','block');}
                barSelect("none");
                dis("songIcons","none");
                vis("speedCover", "visible");
                vis("floater", "visible");
...
function countIn(int,step,loop)
    {   
        ...
        {   step=parseInt(step +1,10);
            if (SOUNDmode=='DRUM COUNT') {document.getElementById('cd').style.visibility='hidden'; }//QQQQ
            ...
            else
            {
                ...
                if (step>BEATS){loop=loop+1;step=1;document.getElementById('cd').style.color='red';}
                document.getElementById('cd').innerHTML=step;
...
function trackPlay(pause)
    {
        ...
        CTO();
        SCROLLstartTime = new Date().getTime();
        if (SOUNDmode !== 'SILENT'&SOUNDmode!=='DRUM COUNT')// {document.getElementById("Audio1").play();} //QQQQ
        {
            if (SOUNDmode =='BACK TRACK'|SOUNDmode =='VOCAL')
            {
        ...
        Yend = false;
        document.getElementById('thinkIcon').src="../../Icons/trans.png"; 
        if (pause>0) {TIMEOUTscroll=setTimeout(function(){SCROLLstartTime = new Date().getTime();scrollEngine(); },pause);
...
function listTempos()
    {  
        ...
        {
            lst ="You cannot change the Tempo of<br>Pre-recorded VOCAL OR BACKING TRACK...";
        }
        else
        {
...
function loadFile(e)
    ...
    } //gets the file//does not update the player until you go there --needs some time so I gave it 2 seconds;
...
function barSelect(bar,msg) 
    {   
        ...
        var arrBar="configuration,information,tempo,tool,key-Selection,sound-track,drumBar,cloud".split(",");
        var j=0;  var ctrl;
...
function receiveARR(divider,keepName)
    {//decode array (or string) and leading elements (put in receiving page)
