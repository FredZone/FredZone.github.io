...
function whodat(){alert ("GLOBAL.ROUTINES.JS")}
...
function lineType(str) {
    var ans = "lyric";
    if (str.substring(0, 7).toUpperCase() == "IREALB:") {
        ans = "irealb";
    } else if (str.substring(0, 4).toUpperCase() == "TAB:") {
        ans = "tab";
...
function fileGetLocal(process) {//process is the name of the process to evaluate the text
    TEMPflag = process;
...
function autoSize(id, fVh, bottom, ht, wt) { //* id//* fontsize(vh)//* text Bottom//* ht//*  % done at  1/1 screen RAT
    if (fVh !== undefined) {
        fVh = parseFloat(fVh / RAT) + 'vh';
        document.getElementById(id).style.fontSize = fVh;
    }
    if (bottom !== undefined) {
        bottom = parseInt(bottom / RAT, 10) + '%';
        document.getElementById(id).style.bottom = bottom;
    }
    if (ht !== undefined) {
        ht = parseInt(ht / RAT, 10) + '%';
        document.getElementById(id).style.height = ht;
    }
    if (wt !== undefined) {
        wt = parseInt(wt * RAT, 10) + '%';
...
function breakStrAtCaps(str) {
    return (str.match(/[A-Z]*[^A-Z]+/g));
} //returns array of words split by in string split with Caps
...
function lTrim(str) {
    var bog = str;
    while (bog.substring(0, 1) == " ") {
        bog = bog.substring(1, bog.length);
    }
    return bog;
}
...
function rTrim(str) {
    var bog = str;
    while (bog.substring(bog.length - 1) == " ") {
        bog = bog.substring(0, bog.length - 1);
    }
    return bog;
}
...
function receiveARR(divider, keepName) { //decode array (or string) and leading elements (put in receiving page)
    ...
    if (keepName === true | keepName === undefined | keepName === '') {
        window.name = oldName;
    }
    return data;
}
