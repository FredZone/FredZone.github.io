...
//PROGRAM and SONG VARS//===========================================================================
    ...
    var SONGlastCustom=0;//0=Key,1=Tempo,2=Sound
...
function bcPct(size){//BigChordPCT
    if (size!==undefined){
        BIGchordSize=1.00;}//default
    else if (BIGchordSize==2){
        BIGchordSize=1.00;}
    else{
...
function setStartBox(){
    return;
    statusMsg("Starting Javascript: Loading Start Times...");
    var j=1;
    var ihtml="<Select><optgroup>\n<option selected>None</option>";
    while (j < 12) {
        ihtml =ihtml +"\n<option>"+ j +"</option>";
        j = j+1; }
    ihtml =ihtml +"\n</optgroup></Select>";
    document.getElementById("startBox").innerHTML=ihtml;}
...
function arrConvert(){//alert('arrConvert()'); //* Setup to walk thru the ARRLines
    ...
    offset ="";
    j=0;
    while(j <= os){offset = offset+"<br>" ;j = j+1;}
...
function listTempos(){  
        ...
        document.getElementById('tempo').innerHTML=lst;//alert(lst);
    }
function newKey(nk){   
...
function statusMsg(msg,bgcolor,marq){ //grey-normal;red-attention Required ;yellow-normal pause or inwork//* xxx could trim to 40 chr
