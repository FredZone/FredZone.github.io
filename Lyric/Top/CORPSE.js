...
function barsAlign(){
    //statusMsg("AUDIO: "+Audio1.currentTime+" --  SCROLL: "+(Ypos/SCROLLpix)*DUR,0)
    statusMsg("================================SCROLL: "+Ypos+"==================================",0)
    statusMsg("=================================AUDIO: "+SCROLLpix*Audio1.currentTime/DUR+"pixels=======================",0)
    //var delta =SCROLLpix*Audio1.currentTime/DUR-Ypos
    //statusMsg("==================================Delta:"+ delta+"pixels=======================",0)
    BARalign=parseInt(SCROLLpix*Audio1.currentTime/DUR,10);
    
    
    //statusMsg("=================================AUDIO: "+Audio1.currentTime +" seconds",0)
    //statusMsg("================================SCROLL: "+DUR*Ypos/SCROLLpix+"seconds=======================",0)
    }
...
//    var newPt
 //   for (i=0;i<ARRbars.length;i++){
 //       newPt=parseFloat(SCROLLpix/MEASURES,10)*ARRbars[i]
 //       ARRbars[i]=parseInt(newPt,10);}
...
function presets(){
    ...
    if (PRESET==="Default Mode"){arrTF="true,false,false,false,false,false,false,false,false,false,false,false,false,true,false,false,false".split(',');
        setLineLimit(90)}
    else if (PRESET==="Perform Mode"){arrTF="true,true,false,true,false,true,false,true,false,false,true,true,false,true,true,false,false".split(',');
        setLineLimit(50)}
    else if (PRESET==="Practice Mode"){arrTF="true,true,false,true,false,false,false,true,false,false,true,true,true,true,true,true,false".split(',');
        setLineLimit(55)}
    else if (PRESET ==="Developer Mode"){arrTF="true,false,false,true,false,false,false,false,false,false,false,false,false,false,false,false,false,true".split(',');
        ...
        //document.getElementById('mySet').selectedIndex=5;selectTune('Abilene')
        selectSet("Test")}
...
window.onload= function(){
    ...
    TIMEOUTcrap= setTimeout(function(){start();},1500);}  
...
function createSetSelector(){ //alert('SetSelector');  //^creates an option box from the file SetList.txt in the top directory
    statusMsg("Building Set Selector...");
...
function createARRlines(content){//^ Make ARRlines, from the text file
    statusMsg("Formatting Download: "+TITLE,'yellow');
    NOTEset=undefined;
    ARRlines ="";
...
/*
function loadServerTrackXXX(){//alert('loading Server Track')
    rollMirror(false)
    statusMsg("Filling any Audio Requirement");
    var delay;
    var src;
    delay =1000;
    document.getElementById('mirror').style.color='transparent'
    ...
    if(SOUNDmode=="SILENT"){
        ...
        if(SOUNDmode=="BACK TRACK"){
            ...
            if(BEATS!=2 & BEATS!=4 & BEATS!=8 &BEATS!=16 & icon=='none.png'){// kick out if not 4/4 based and no trackPicked
                document.getElementById('buttonTrack2').src="../../Icons/transTrackNo.png";
                AUDfail =true;
                statusMsg("CLICK TRACK not supported at "+BEATS+ " beats per bar")
                durCalc();
                return;}//^ 4/4 tracks below here=============================================
            if(SOUNDmode=="DRUM ROCK"){
               icon ="transRock.png";
...
//-----------------------------------------------  TOP  OF CHORD        
            ...
            if (BARsync===true){
                ...
                while(n<NewLine.length){
                    if (NewLine[n]==="<"){htm=true}//strip color
                    else if (NewLine[n]===">"){htm=false}
                    else if (htm===true) {htm=true}
                    else if(htm!=true){
                        if(NewLine[n]=="|"){
                            if (firstChord===true){
                               //barLine=barLine+"<a id=\'b"+bars +"\' onclick=\"document.getElementById(this.id).style.backgroundColor=\'green\'\" style=\"backgroundColor:lightgreen;\">|"
                                barLine=barLine+"<a id=\'b"+bars +"\' style=\"backgroundColor:transparent;\">|"
                                firstChord=false}
                            else{
                                barLine=barLine+"</a><a id=\'b"+bars+"\' style=\"backgroundColor:transparent;\">|"}
                            bars++}
...
//-------------------------------------------- 
                ...
                if(j+1<ARRlines.length) {
                    ...
                    var extraType=lineType(extraLine)
                        if(lineType(extraLine)==='lyric'){//||lineType(ARRlines[j+1])!==undefined||lineType(ARRlines[j+1])!==undefinedlines[j+1];
                            ...
                            j++;}}  //incase there is a 2nd lyric line
    ...
    while(j <= os-3){offset=offset+"<br>" ;j++;}
    ...
    offset2 =""; j =1;
    while (j < parseInt(((visLines)/2)-1,10)){offset2=offset2+"<br>";j++;}
...
function scrollSetup(img){//alert('scrollSetup()');//^ get the song data,songHeight,iframeHeight,Duration,ScrollConstant Run after you set the content of page
    ...
    if(CLOCK===true) {
        vis("clock","visible");}else{vis("clock","hidden");}
...
function scrollEngine(){//^ the actual scrolling routine keep it simple* before it starts SCOLLstartTime,SCROLLkon must be set
    //statusMsg("Ypos: "+parseInt(Ypos,10)+' px  BAR:'+BAR+" MODE:?")
    var mode ="Std"
    var newPos;
    Ypos=document.getElementById("Tune").scrollTop;
    if(Ypos==TOPlast) {SS++}else{SS=0;TOPlast=Ypos}
    if(Yend===true){ statusMsg("Yend: "+Yend,0);                              
        if(LOOPER===false){endSong();}else{endLoop();}}         
    else{
        if (BARsync===true & Audio1.duration>1) {
            if (Ypos<=1){document.getElementById('b0').style.backgroundColor='lightgreen'}
               // statusMsg("BAR: "+BAR+" starting scroll...",0);
               // document.getElementById('b'+BAR).style.backgroundColor='lightgreen'
            //    BARSTOP=ARRbars[0]/MEASURES*DUR;}
            else if (Audio1.currentTime>BARSTOP){
                statusMsg("BAR:"+BAR+" will end at "+BARSTOP + " sec of "+DUR+ " sec  >>>>>>>>>>>>>>>>>>>>>>>>",0)
                BAR=BAR+1;
                BARSTOP=ARRbars[BAR]/MEASURES*DUR;
                document.getElementById('b'+BAR).style.backgroundColor='lightgreen'}}
        /*==========================================================================================
        if (newPos>BARSTOP){
            statusMsg("BAR:"+BAR+" ends at "+BARSTOP +" @ "+Ypos +" >>>>>>>>>>>>>>>>>>>>>>>>")
            BAR=BAR+1;
            BARSTOP=ARRbars[BAR];}
        document.getElementById('b'+BAR).style.backgroundColor='lightgreen'}
        =========================================================================================*/
        ...
        statusMsg("-----Ypos: "+parseInt(Ypos,10)+' px  BAR:'+BAR,0)
        TIMEOUTscroll=setTimeout(function(){scrollEngine();},SCROLLkon);}

function closeSong(){
    alert('dont know why?')//^ used to indicate audio end (needed to remove listener)
...
function endSong(){
    ...
    var wait;
    if(SOUNDmode=='BACK TRACK'|SOUNDmode=='VOCAL'){wait=100;}
...
function zzendSong(){
    if (Ypos+1>=SCROLLpix) {
        statusMsg("Ending Song...",0)
        var wait=3000;
        TIMEOUTwait=setTimeout(function(){passCommand('End');},wait);}
    else{statusMsg("SCROLL wont stop Ypos:"+Ypos+" not equal SCROLLpix " +SCROLLpix)}}
        /*
...
function XXXXpassCommand(cmd){ //^ configures the play screen to match the play status(moves, displays and hides icons )
    statusMsg(cmd,0)
    if(cmd == "DelayedPlay"|cmd == "Scroll"| cmd == "Ready" | cmd == "Pause"| cmd == "Think"| cmd == "End"){
        dis('Audio1','none');
        vis('altSong','hidden');
...
function noteTranspose(note,steps){//^ must be sure to add # or b beforce calling the routine  
    var noteNew = "?";
    var arrScale="A,Bb,B,C,C#,D,Eb,E,F,F#,G,Ab,A,Bb,B,C,C#,D,Eb,E,F,F#,G,Ab".split(",");  //^ make array
    ...
    for (i = 13; i <=11; i++) {//^  get the position of the first letter starting at 13 and work up
