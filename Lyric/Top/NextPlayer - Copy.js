...
//*PROGRAM and SONG VARS//
    ...
    var ARRsoundModes=("SILENT/nBACK TRACK/nCLICK TRACK/nVOCAL/nDRUM ROCK/nDRUM COUNTRY/nMETRONOME").split("/n");
...
//* TIMEOUT CONSTANTS
    ...
    //var TIMEOUTintro=100; dont know
...
//* CONFIGURATION
    ...
    var ARRpresets="CAPS,FULLscreen,LEFTborder,CLOCK,TEXT,SHADE,NOTES,SETnotes,POPnotes,TECHnotes,TBD,COUNTin,CLICKER,BREAKlines,BIGchords,LOOPER,METRO,BARsync".split(',');
    var TBD= false;
    var BACKdrop=false;
    var BARsync=false
    var BIGchords=false;
...
window.onload = function() {
    ...
    ARRstatusLog = "<pre>========================BOOTING=================</pre>".split('-');
    STATUSmon = 1;
    BOOT = true;
...
function monitorBoot() { //initiated by the user at boot
    ...
    statusMonitor(2);
    statusMsg('Boot monitor initiated by User');
    start();
}
...
function durCalc() {
    ...
    if (SOUNDmode === "BACK TRACK" | SOUNDmode === "VOCAL" & document.getElementById('Audio1').duration > 5000) {
        DUR = Audio1.duration;
...
function scrollSetup(img) { //alert('scrollSetup()');//^ get the song data,songHeight,iframeHeight,Duration,ScrollConstant Run after you set the content of page
    ...
    if (BOOT === true) {
        ...
        if (agent=='Chrome') {
            mesg='WARNING: Tunes WON\'T Restart or Loop using KWS server and Chrome (local host on Phone)';
            msgColor = 'red'};
    ...
    //alert("OLD BEATS: " + BEATS + " >> " + METRObeats + "\nOLD BPM:" + BPM + " >> " + METRObpm + "\nMET CHANGE: " + old + ">>" + MET)
