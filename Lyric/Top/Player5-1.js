    var REV ="5.1"//this revision
    var JSname="Player5-1.js"//java script file
    var DEBUGboot=false;//change to true to view boot step by step
...
//* SCROLLdata CONSTANTS   ==============================================================
    var TOPlast;
    var SS=0;  //SS=scroll stopper...counts cylces of non scroll indicating end of tune
...
//*REQUIRED TO LOG Status Mssages,See Developer Section
    var BOOTlog=true
    var DEBUG=true
    var LOG=40;//length of Log
    var ARRstatusLog="Opening Debug Log...".split('-');
    var MSGcount=0;
  

//*Bogus Functions===========================================================
function sleep(miliseconds) {//stops execution 
   var currentTime = new Date().getTime();
   while (currentTime + miliseconds >= new Date().getTime()){}}

function stepThroughBoot(){
    document.getElementById("watchBoot").display='none';  
    document.getElementById("watchBoot").blur();  
    DEBUGboot=true;
    alert("BOOT \n"+ARRstatusLog.join('\n'));
    }
...
window.onload = function(){
    document.getElementById("watchBoot").focus();
    document.getElementById("msg").innerHTML="SLEEPING...";
    
    BOOTlog=true;//collect a debug log to be reviewed after boot 
...
//function continueLoad(){  
    if (DEBUGboot===true) {ARRstatusLog="=========================\nSTEP by STEP BOOT\nPRESS ENTER FOR EACH STEP".split('/')}
    var arr;
    revDates();
    MSGlast="MSG ERROR";
    DEBUG=true;//for the boot
    ...
    ARRsongSettings=('C;92;transCircleWhite.png').split(';');//a starting point for the songSettings 
    //^ set up sound track selector    XXX could be hard code in html page
...
function createSetList(){//alert('createSetList()');//^ creates the setlist option box and selects tune 0
    statusMsg("Formating " +SETname +" playlist...","yellow");
    var a;
    var lst ="<X2><optgroup>";
    j =0;
...
function openSong(){//alert('openSong()');//opens a song with title, if its on server, not on list============EEE
    statusMsg("Requesting a Title...",0);
...
function selectTune(titl){//alert('selectTune('+titl+')');//^ Entry Point using the TITLE (or extended title) to download tune
    statusMsg("Setting up for "+titl +"...",'lightgrey');
    directCopy(titl)
    barSelect('none')
...
function createARRlines(content){//^ Make ARRlines, from the text file
    statusMsg(TITLE+": Creating Song Array...",'yellow');
    NOTEset=undefined;
    BARS=0;
    ARRlines ="";
...
function loadServerTrack(){//alert('loading Server Track')
    statusMsg("Checking for mp3 track...",0);
    var delay;
    var src;
    delay =1000;
    document.getElementById('mirror').innerHTML="---";
    document.getElementById('mirror').style.backgroundImage="url('../../Icons/resetSpinner.gif')";    
    ...
    src ="";
    if(SOUNDmode=="SILENT"){
        document.getElementById('buttonTrack2').src="../../Icons/transSilent.png";
        ...
        var x = setTimeout(function(){//^ time to get the mp3 [arbitrary]
            ...
            if (isNaN(DURmp3)===false ){//^ track successfully loaded
                ...
                blinker('buttonTrack2',30)}
            document.getElementById('buttonTrack2').src="../../Icons/"+icon;
            ARRsongSettings.splice(2,1,icon);
            document.getElementById('thinkIcon').src="../../Icons/trans.png"; 
            durCalc();},delay); }}//^ time to load mp3

function durCalc(){ //alert(' durCalc()');
    //badCode()use to insert errors in the program to test debugger etc...
...
function arrConvert(){//alert('arrConvert()'); //^ Setup to walk thru the ARRLines
    statusMsg( TITLE +': Converting Array for Presentation...',"yellow");
    ...
    if (LONGLINE<60){fsize=3.8-(LONGLINE-30)*0.02500;}
    else if (LONGLINE<75){fsize=3.3-(LONGLINE-40)*0.02500;}
    else{fsize=2.14-(LONGLINE-75)*0.025;}//^ emperically derived        
...
function scrollSetup(img){//alert('scrollSetup()');//^ get the song data,songHeight,iframeHeight,Duration,ScrollConstant Run after you set the content of page
    ...
    autoSize("songBar2",3.5,undefined,undefined,undefined);
    autoSize("information",undefined,undefined,undefined,undefined);
    autoSize("tool",undefined,undefined,undefined,undefined);
    ...
    if(BOOT===true){
        ...
        statusMsg('<X2><<<<<<<<<<<<<<<>BOOT LOG<>>>>>>>>>>>>>>></X2>',0)
        DEBUG=false}
    else{statusMsg(mesg,msgColor);}
    BOOT = false;
    document.getElementById('thinkIcon').src="../../Icons/trans.png";
    LOOPend=SCROLLpix;
    ...
    DEBUGboot=false;//Normal Debug now available
    showSetting();}
...
function CTO(){//^ clear all Timouts except blink
    statusMsg("Clearing '6 Time Outs'",0);
...
function passCommand(cmd){ //^ configures the play screen to match the play status(moves, diplays and hides icons )
    statusMsg("Processing Command: "+cmd,0)
...
function scrollRate(factor){
    statusMsg("Changing Scroll Rate "+factor*100+"%",0)
...
function trackPlay(pause){
        ...
        statusMsg('Start Play with '+pause+" pause",0)
        AUDend =true;
        CTO();
        SCROLLstartTime = new Date().getTime();
...
function setSoundMode(mode,icon){
    ...
    showSetting();
    SOUNDmode = mode;
    statusMsg("Set sound mode to " +mode,0)
    loadServerTrack();}
        
function setCustom(VAR,val,action){
    statusMsg("Custom Setting :"+ VAR +"; "+val+"; "+action)
    if (!val){
    ...
    if (VAR=='DEBUG'){
        if (BOOTlog ==true) {BOOTlog=false; dis('buttonDebug','block');notePopUp('<pre>'+ARRstatusLog.join('\n')+'</pre>','2vw','black','white','WTF?','DEBUG LOG','Viewed the Debug Log')}
        else if(DEBUG===true){dis('buttonDebug','block');ARRstatusLog="<X2>>>>>>>>>>>>>>DEBUG LOG INITIATED<<<<<<<<<<<<</X2>".split('-');MSGcount=0;}
        else{dis('buttonDebug','none')}        }
...
function showSetting() {// alert('showSetting('+num+')')
    ...
    if (num===0) {//set Icon to geneneric & text to key and move Icon down
        ...
        document.getElementById('mirror').style.backgroundImage="url('../../Icons/transCircleWhite.png')"
        url='url(../../Icons/transCircleWhite.png)'
        changeBackgroundImage('mirror', url)}
    else if (num===1) {//set Icon to geneneric & text to BPM and move Icon down
        document.getElementById('speed2').innerHTML=BPM;
        document.getElementById('mirror').innerHTML=BPM;
        document.getElementById('mirror').style.backgroundImage="url('../../Icons/transCircleWhite.png')"}  
    else{//set icon and put it on top     
        changeBackgroundImage('mirror','../../Icons/'+ARRsongSettings[2]);
        document.getElementById('mirror').style.color='transparent';
        document.getElementById('mirror').innerHTML=".";
        document.getElementById('buttonTrack2').src="../../Icons/"+ARRsongSettings[2];}}
...
function statusMsg2(msg,bgcolor,marq){    
  var clr="black";
  if(msg===null){msg=MSGlast;}
    else{MSGlast = msg;}
    if (document.getElementById('splash').style.display=="none") {bgcolor = "transparent"}
...
function statusMsg(msg,bgcolor,marq) {// COLOR SCHEME: light grey=normal;red=problem ;yellow-pause or inwork; green=Ready
    var clr="black";
    var boot="XXX"
    if (DEBUGboot===true){//record the boot 
        alert(ARRstatusLog.join('\n'));}
    if (DEBUG===true){//keep a running 40 element log if DEBUG is activated
        if (ARRstatusLog.length>40 ){ARRstatusLog.pop()}
        if(bgcolor===0){//Log only, no display
            ARRstatusLog.splice(0,0,stdTime("",true)+"   <X8>"+msg+"</X8>");
            return;}
        else{ARRstatusLog.splice(0,0,stdTime("",true) +"   "+ msg);}}//Log and Display
    if(msg===null){msg="*"+MSGlast;}
    else{MSGlast = msg;}
