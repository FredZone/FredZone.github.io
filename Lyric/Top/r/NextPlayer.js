...
function ZZZZsizeTuneSelect(){
   dis('bigSelector','none')
   if (BIGselect==false) {
      BIGselect=true;
      statusMsg("Creating Small Selector...");
      document.getElementById('BSicon').src='../../Icons/blackTextSmall.png';
      document.getElementById('BSbutton').style.display='block';
   }else{
      statusMsg("Creating Small Selector...");
      BIGselect=false;
      document.getElementById('BSbutton').style.display='none';
      //document.getElementById('BSicon').src='../../Icons/blackTextLarge.png';
   }
}
//^MAIL FUNCTIONS==========================================
function getSelectionText() {//used in mail
    var text = "";
...
function selectPlaylist(set) { //alert('selectPlaylist('+set+')');  //^ Selects your set by its name
    ...
    TUNEnum = 0
    createPlayListSelector(0);//the ARRAY of songs in the set should be playlist
}

function createPlayListSelector(sel) { //alert('createSetList()');//^ creates the setlist option box and selects tune 0
   statusMsg("Formating Playlist Selector: " + SETname + ", Tune Number:" + TUNEnum, "yellow");
   if (sel == undefined) {
      sel = 0
   }
   document.getElementById('BSbutton').style.display = 'none'
   ...
   document.getElementById("mySet").selectedIndex = sel;
   if (BIGselect == true) {
      statusMsg("Formating BIG Playlist and Selecting Default Song:" + TUNEnum, "yellow");
      var i = 0;
      str = ""
      while (i < PLAYlist.length) {
         str = str + "<div id='BS" + i + "' onclick=\"selectTuneNo(" + i + ")\" style='position:relative;border:solid;height: overflow:hidden;5vh;line-height:110%;width:100%;font-size:4vh;'>" + (i + 1) + ":  " + PLAYlist[i].split('|')[0] + "</div>";
         i = i + 1;
      }
...
/*   NEXT TUNE    XXXXXX
function XXXnextTune(delta) { //alert("nextTune("+delta+")");//^ Entry point if you have selected the next tune in the list by direction 1,0,-1
   statusMsg("Incrementing tune "+TUNEnum+" by " & delta & " step(s)...", "yellow");
   screenFormat("Think");
   barSelect('none');
   if (PLAYlist[TUNEnum + delta]) { //^ if next tune exists go to it
     i= (TUNEnum + delta);
   }
...
function selectTuneNo(x){
   statusMsg('Selecting Tune '+x+' (by number)')
   alert(x)
   selectTune(PLAYlist[x])
}

function selectTune(titl) {
   statusMsg("Selecting "+titl)
   TITLEplus=titl
   document.getElementById('mySet').selectedValue=titl;
   
   dis('bigSelector', 'none');
   dis('backtracker', 'none'); //SIMPLIFY
   if (BIGselect==true) {
      document.getElementById('BS' + TUNEnum).style.color = 'red';
     // var sk =(TUNEnum/PLAYlist.length)*(document.getElementById('BB').clientHeight)
     // document.getElementById('BS').scrollTop=sk;
   }
   ARRtitle = titl.split("|");
...
function presets(action) {
   ...
   if (PRESET === "Default Mode") {
      ...
      //var ARRpresets="CAPS,FULLscreen,LEFTborder,CLOCK,TEXT,SHADE,NOTES,SETnotes,POPnotes,TECHnotes,LIVEnotes,COUNTin,TBD2,BREAKlines,BIGchords,LOOPER,METRO,BARsync,BIGselect".split(',');
...
function setCustom(VAR, val, action) {
   ...
   if (action === undefined | action === null | action === '' | action === ' ') { //clean up undefined
      action = 'ERROR';
      statusMsg("ERROR...setCustom Process Terminated for undefined action")
      return;
   ...
   //if (VAR == 'TBD') {
   //   if (TBD == false) {
   //      document.getElementById("imgTBD").src = "../../Icons/off.png"
   //   }
   //}
