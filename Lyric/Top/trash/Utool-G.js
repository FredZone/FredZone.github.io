...
var VID;
var VIDnote;
//the index of the video in ARRvideos
//var VIDtitle;
...
function collectionsGet(type){//creates ARRcollections from server or local
  if (type=='server') {
    FILEsource="WEBSITE";
...
function vidsGet(vid,loop){//creates ARRvideos passes default value to selector and default loop
...
//path=path+"?amp;showinfo=0&enablejsapi=1&rel=0"; //WORKS but shows suggestions
  ...
  if (UTback!==undefined) {//Load a backtrack to player2
    ...
    document.getElementById('player2').src="https://www.youtube.com/embed/"+ UTback;}
  else{
...
function loopSelect(loop) {//alert('loopSelect('+loop+')');//sets the default values of a loop     
  ...
  document.getElementById('tabBar').innerHTML='<pre>'+LOOPtab.split('$').join('<br>')+'</pre>';
