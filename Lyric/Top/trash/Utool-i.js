...
function collectionsGet(type){//alert('collectionsGet('+type+')');//creates ARRcollections from server or local
  statusMsg('Getting File from'+type+'& Building Selector ');
  dis('blocker','block');
  if (type=='server') {
    FILEsource="WEBSITE";
    ARRcollections=fileDownload('UtubeUtool.txt').split("\n");
    COL = ARRcollections.indexOf("Default Videos");
    if (COL===false|COL===undefined|COL===''|COL===-1) {COL=0;}
...
function colSelect(col){//alert('colSelect('+col+')');//creates the ARRcollections select box
  ...
  statusMsg('Selecting Collection item #'+col+1);
...
function localFile(){//alert('localFile()');
    ...
    mode('Loading...');
    //ARRvideos="No Note;nveEtVqYyvw#Monkey Video;1#5#Fake Loop;1#5#crap\nnveEtVqYyvw#Monkey Video2;1#5#Fake Loop;1#5#crap".split('\n');
    COLnote='Bogus Video\nUsed To Test';
...
function vidsGet(vid,loop){//alert('vidsGet('+vid+')');//creates ARRvideos passes default value to selector and default loop
  statusMsg('Loading Video Index and selecting #'+ vid);
  dis('blocker','block');
  ...
  document.getElementById('fNote').innerHTML=FILEname+': '+COLnote.split('\n')[0];
  document.getElementById('editColNote').value=COLnote;
  document.getElementById('colNote').value=COLnote;
...
function vidSelect(vid,loop){
  ...
  UTnote=temp[2];
  UTback=temp[3];
  document.getElementById('vNote').innerHTML=UTtitle+': '+UTnote.split('$')[0];
  ...
  path=path+'&rel=0';//this crap should work but does not hide utube suggestions
  dis('blocker','block');
...
function onPlayerReady(event){
    statusMsg('PLAYER READY xxxxxxxxxxxxxxxxxxxxxxxxxxx')
    loopsGet(LOOP);
    }
...
function loopSelect(loop) {//alert('loopSelect('+loop+')');//sets the default values of a loop     
  ...
  document.getElementById('lNote').innerHTML=LOOPname+': '+LOOPnote.split('$')[0];
...
function  final(){//alert(' final()');
  ...
  mode('Ready...');
  if(player.getCurrentTime()>0){mode('Paused...');}else{mode('Cueing...');}//zzz
  }
...
function vidUpdate(){//alert('loopCapture('+LOOP+')');//captures current video&loop and adds it to this spot//zzzThis works
  ...
  statusMsg('A-0');
  var cn=document.getElementById('editColNote').value;
 
  COLnote=cn;statusMsg('A-1');
  //document.getElementById('editColNote').value=fn;
  protect(true);
  //fn=encodeLines(cn,';');
  vo=encodeLines(vo,'$');
  n=encodeLines(n,'$');
  t=encodeLines(t,'$');  
  clearTimeout(TIMEOUTtime);statusMsg('A-2');
...
alert(FILEname);
  ...
  alert(ARRvideos.length);
alert('UNCUT ARRvideos;=============================\n' +ARRvideos);
  COLnote=ARRvideos[0];
alert('COLnote;'+COLnote);
...
function statusMsg(msg,bgcolor,marq)//* xxx could trim to 40 chr
    {   // COLOR SCHEME: light grey=normal;red=problem ;yellow-pause or inwork; green=Ready
        ...
        if(ARRstatusLog.length>=31){ARRstatusLog.splice(30,1);}
